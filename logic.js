$(document).ready(function() {
    ...
    if (app == "app") {
        $(
            ".headerWrapper, .footerWrapper, .contentWrapper h2"
        ).remove();

        $(".mortgage-calculator").prepend(
            '<div class="col-12 app-header"><img id="app-logo" src="/images/ApprovedLogo.jpg"><div class="title">Mortgage Calculator</div></div>'
        );
...
function MM_findObj(n, d) {
    ...
    if (
        ...
        d =
            parent.frames[n.substring(p + 1)].document;
        n = n.substring(0, p);
    }
...
function CMHCRate(PercentDown, Amortization) {
    ...
    PercentFinanced = Number(
        100 - Number(PercentDown)
    );

    var total_rate = 0.0;
    total_rate = 0;

    if (Amortization == 40) total_rate = 0.6;
    else if (Amortization == 35) total_rate = 0.4;
    else if (Amortization == 30) total_rate = 0.2;

    if (PercentFinanced <= 80) total_rate = 0;
    else if (PercentFinanced <= 85)
        total_rate = total_rate + 2.8;
    else if (PercentFinanced <= 90)
        total_rate = total_rate + 3.1;
    else if (PercentFinanced <= 95)
        total_rate = total_rate + 4.0;
...
function mortgagePayment(nAmount, nRate, nAmort) {
    ...
    return (
        ...
        ((1 /
                (Math.pow(
                        1 + nRate / 200,
                        1 / nPaymentsPer6Months
                    ) -
                    1)) *
            (1 -
                Math.pow(
                    1 + nRate / 200, -nAmortMonths / nPaymentsPer6Months
                )))
    );
}
...
function currencyString(nNumber) {
    ...
    var str =
        "" +
        Math.round(nNumber * Math.pow(10, nNumDec));
    while (str.length <= nNumDec) {
        str = "0" + str;
    }
    var decpoint = str.length - nNumDec;
    var result = commaFmt(
        str.substring(0, decpoint) +
        "." +
...
function doCalcOnForm(form) {
    ...
    if (checkForm(form)) {
        var nCounter = 0;
        for (
            nCounter = 1; nCounter <= 3; nCounter++
        ) {
            ...
            tmp =
                (parseFloat(dp) / parseFloat(price)) *
                100;

            form["percent_down" + nCounter].value =
                Math.round(tmp * Math.pow(10, 8)) /
                Math.pow(10, 8);

            // calculate downpayment
            nDownPayment =
                filterNum(form.io_price.value) *
                (Number(
                        form["percent_down" + nCounter].value
                    ) /
                    100);
            nFirstMortgage =
                filterNum(form.io_price.value) -
                nDownPayment;
            form["first_mortgage" + nCounter].value =
                currencyString(nFirstMortgage);
            nCMHCPremium =
                (CMHCRate(
                        Number(
                            form["percent_down" + nCounter].value
                        ),
                        form.io_amortization.options[
                            form.io_amortization.selectedIndex
                        ].value
                    ) *
                    nFirstMortgage) /
                100;
            form["cmhc" + nCounter].value =
                currencyString(nCMHCPremium);
            nTotalFinancing =
                nFirstMortgage + nCMHCPremium;
            form["total_financing" + nCounter].value =
                currencyString(nTotalFinancing);
            nPayment = mortgagePayment(
                nTotalFinancing,
                Number(form.io_interest.value),
                Number(
                    form.io_amortization.options[
                        form.io_amortization.selectedIndex
                    ].value
            ...
            form["monthly_payment" + nCounter].value =
                currencyString(nPayment);
            console.log(
                "heat: " +
                Number(filterNum(form.io_heating.value))
            );
            nExpenses =
                Number(filterNum(form.io_heating.value)) +
                Number(filterNum(form.io_taxes.value)) /
                12 +
                Number(filterNum(form.io_condo.value));
            form["expenses" + nCounter].value =
                currencyString(nExpenses);
            nTotal = nExpenses + nPayment;
            form["total_payment" + nCounter].value =
                currencyString(nTotal);
            nIncome =
                (nTotal * 12) /
                (Number(form.io_gds.value) / 100);
            form["income_required" + nCounter].value =
                currencyString(nIncome);
        }
    } else {
        clearResults(form);
    }
...
function formatRealNum(theNum, decplaces) {
    ...
    var str = Math.round(
        parseFloat(filterNum(theNum.value)) *
...
function formatNum(theNum, decplaces, addcommas) {
    ...
    if (decplaces == 0) {
        var fmtdnum = formatInteger(
            theNum,
            decplaces
        );
    } else {
        var fmtdnum = formatRealNum(
            theNum,
            decplaces
        );
    }
...
function scrub_String(
    ...
    for (i = 0; i <= theStr.length; i++) {
        ...
        if (charsAllowed.indexOf(theChar) != -1)
            ...
            specialCharAt =
                charsAllowedOnce.indexOf(theChar);
            if (specialCharAt != -1) {
                result += theChar;
                // remove the char from the list so it will not be allowed
                charsAllowedOnce =
                    charsAllowedOnce.substring(
                        0,
                        specialCharAt
                    ) +
                    charsAllowedOnce.substring(
                        specialCharAt + 1,
                        charsAllowedOnce.length
                    );
...
function commaFmt(numEle) {
    ...
    var jInt = 0;
    for (
        var iInt = tempStr.length - 1; iInt >= 0; iInt--
    ) {
        jInt++;
    ...
    if (decInt != -1) newStr = newStr + decStr;
    if (isNeg) newStr = "-" + newStr;
...
function format_Real(theNum, decplaces) {
    ...
    str = Math.round(
        parseFloat(str) * Math.pow(10, decplaces)
    );
    str = "" + str;
    while (str.length <= decplaces) {
        str = "0" + str;
...
function format_Number(
    ...
    else var fmtdnum = format_Real(str, decplaces);

    if (addcommas) fmtdnum = commaFmt(fmtdnum);
    //	if (isNaN(fmtdnum))
    //		return "";
    return fmtdnum;
}
...
function validateNum(
    ...
    var tmpFloat = parseFloat(
        filterNum(theNum.value)
    );
...
function str_Empty(theFormObj) {
    if (theFormObj.value + "" == "null")
        return true;
    else if (theFormObj.value == "") return true;
    return false;
}
...
function popup_Empty(theMenuObj) {
    var theSelection = theMenuObj.selectedIndex;
    if (theSelection == -1) return true;
    else if (
        theMenuObj.options[theSelection].value == ""
    )
        return true;
    else return false;
}
...
function field_Empty(theFormObj) {
    ...
    else return str_Empty(theFormObj);
}
...
function numberRange_OK(fromNumObj, toNumObj) {
    ...
    var tmpFromFloat = parseFloat(
        filterNum(fromNumObj.value)
    );
    var tmpToFloat = parseFloat(
        filterNum(toNumObj.value)
    );
...
function scrub_multi_value_field(
    fieldContents,
    minWildcardPosition
) {
    ...
    if (
        fieldContents == "" ||
        fieldContents == "undefined"
    )
        return "";
    if (
        minWildcardPosition == "" ||
        minWildcardPosition == "undefined"
    )
    ...
    if (fieldContents.indexOf(",") == -1) {
        ...
        var fieldContents_Array =
            fieldContents.split(",");

        for (
            var sc_x = 0; sc_x < fieldContents_Array.length; sc_x++
        ) {
            if (fieldContents_Array[sc_x] == "")
                continue;
            if (
                fieldContents_Array[sc_x].charAt(0) == " "
            )
                fieldContents_Array[sc_x] =
                fieldContents_Array[sc_x].substring(
                    1,
                    fieldContents_Array[sc_x].length
                );
            if (
                fieldContents_Array[sc_x].charAt(
                    fieldContents_Array[sc_x].length - 1
                ) == " "
            )
                fieldContents_Array[sc_x] =
                fieldContents_Array[sc_x].substring(
        ...
        // remove commas from the end of the string
        for (
            sc_i = result.length - 1; sc_i >= 0; sc_i--
        ) {
            if (result.charAt(sc_i) != ",") break;
...
function checkPostal(theObject, removeSpace) {
    ...
    for (i = 0; i <= 5; i++) {
        ...
        if (i == 1 || i == 3 || i == 5) {
            if (!isNum(u)) failed = true;
        } else {
            if (isChar(u)) u = u.toUpperCase();
            else failed = true;
        }
        var b = b + u;
    }
    if (!removeSpace)
        b =
        b.substring(0, 3) + " " + b.substring(3, 6);
...
function replace_Char(
    ...
    while (
        (x = theStr.indexOf(unwantedChar)) != -1
    ) {
        theStr =
            theStr.substring(0, x) +
            wantedChar +
            theStr.substring(
                x + unwantedChar.length,
                theStr.length
            );
    }
    return theStr;
...
function scrub_String_Unwanted(
    theStr,
    charsUnwanted
) {
    // eliminate all unwanted chars
...
function parse_Wildcards(
    ...
    if (minWildcardPosition == 0) {
        ...
        if (
            ...
            alert(
                "A wildcard can only be used after character " +
                minWildcardPosition +
                " in this field."
            );
            theStr = replace_Char("*", "", theStr);
            ...
            if (x != -1) {
                tmpStr = theStr.substring(
                    x + 1,
                    theStr.length
                );
                if (tmpStr.indexOf("*") != -1) {
                    alert(
                        "Only one * wildcard is allowed in this field."
                    );
                    theStr = replace_Char("*", "", theStr);
                    theStr = replace_Char("?", "", theStr);
                }
...
function scrub_Text(
    ...
    theStr += "";
    if (theStr == "null" || theStr == "") return "";
    for (var i = 0; i < theStr.length; i++) {
        if (theStr.charAt(0) != " ") break;
...
function scrub_Time(theStr) {
    ...
    var p,
        pm = "";
    str = str.toUpperCase();
    if ((p = str.indexOf("PM")) >= 0) {
        pm = "PM";
        str = str.substring(0, p);
    }
    if ((p = str.indexOf("AM")) >= 0) {
        pm = "AM";
        str = str.substring(0, p);
    }
    ...
    if (d.getTime()) {
        if (pm == "AM" && d.getHours() == 12)
            d.setHours(0);
        if (pm == "PM" && d.getHours() == 0)
            d.setHours(12);
